...
@interface SHBSpotCertIssueStep2ViewController ()
{
    int certIndex;
...
@implementation SHBSpotCertIssueStep2ViewController
...
- (void)viewDidLoad
{
    ...
    }else
    {
        ...
        self.strBackButtonTitle = @"인증서 암호입력";
    }
    ...
    //인증번호 유효시간 경과 오류를 받는다
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(authCodeExpire) name:@"notiServerError" object:nil];
}
...
- (void)authCodeExpire
{
    ...
    if ([SHBUtility isFindString:AppInfo.serverErrorMessage find:@"인증번호 유효시간 경과"])
    {
        [[NSNotificationCenter defaultCenter] removeObserver:self];
