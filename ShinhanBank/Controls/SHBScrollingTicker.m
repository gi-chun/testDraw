...
@interface SHBScrollingTicker() <UIScrollViewDelegate> {
...
@implementation SHBScrollingTicker
...
- (void) beginAnimationWithViews:(NSArray *) views
    ...
    if (!playOption) [self pauseAnimation];
}
...
- (void)executeSlideAnimation:(UIView *)labels Direction:(LPScrollingDirection)direction{
	if (isAnimating) [self endAnimation:NO];
	lazyLoadingHandler = nil;
	animationCompletitionHandler = ^(NSUInteger loopsDone, BOOL isFinished) {};
	numberOfLoops = 0;
	scrollViewDirection = direction;
	scrollViewSpeed = kLPScrollingAnimationPixelsPerSecond;
	...
	NSMutableArray *arrViews = [[NSMutableArray alloc] initWithCapacity:0];
	[arrViews addObject:labels];
	[self layoutTickerSubviewsWithItems:arrViews];
	[arrViews release];
	
	if (self.frame.size.width >= labels.frame.size.width){
		[self stopAnimation:labels.frame.size.width - 5];
		[self pauseAnimation];
	}else{
		[self beginAnimation];
	}
...
- (CGPoint) startOffset {
    ...
    if (scrollViewDirection == LPScrollingDirection_FromRight)
        startOffset = CGPointMake(-scrollView.frame.size.width+stopPosition, 0);
...
- (void) stopAnimation:(int) labelWidth;
{
    stopPosition = labelWidth;
}
...
#define kLPScrollingTickerLabelItem_Title_Font          [UIFont boldSystemFontOfSize:15.0f]
#define kLPScrollingTickerLabelItem_Description_Font    [UIFont systemFontOfSize:15.0f]
...
- (id) initWithTitleCustom:(NSString *) title description:(NSString *) description colorType:(int) colorType {
    self = [super initWithFrame:CGRectZero];
    if (self) {
        titleLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        titleLabel.font = kLPScrollingTickerLabelItem_Title_Font;
        ...
        // 0: default 1:[UIColor colorWithRed:244.0f/255.0f green:239.0f/255.0f blue:233.0f/255.0f alpha:1.0f] 2:[UIColor colorWithRed:44.0f/255.0f green:44.0f/255.0f blue:44.0f/255.0f alpha:1.0f] 3:
        if (colorType == 1) {
            [descriptionLabel setTextColor:RGB(255, 255, 255)];
            [titleLabel setTextColor:RGB(255, 255, 255)];
        } else if (colorType == 2) {
            [descriptionLabel setTextColor:RGB(44, 44, 44)];
            [titleLabel setTextColor:RGB(44, 44, 44)];
        }
...
- (id) initWithLabelStyle:(NSString *)title FontSize:(CGFloat)fontSize TextColor:(UIColor*)txtColor{
	self = [super initWithFrame:CGRectZero];
	if (self){
		titleLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        titleLabel.font = kLPScrollingTickerLabelItem_Title_Font;
		...
		if (txtColor) {
			[descriptionLabel setTextColor:txtColor];
