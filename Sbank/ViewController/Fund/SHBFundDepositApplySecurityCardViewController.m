//
//  SHBFundDepositApplySecurityCardViewController.m
//  ShinhanBank
//
...
#import "SHBFundDepositApplySecurityCardViewController.h"
#import "SHBFundDepositApplyCompleteViewController.h"
#import "SHBFundService.h"
#import "SHBScrollingTicker.h"

@interface SHBFundDepositApplySecurityCardViewController ()
{
...
@implementation SHBFundDepositApplySecurityCardViewController
...
//        [NSString stringWithFormat:@"%@원", [data_D6020 objectForKey:@"거래금액"]];
    } else {
...
- (void)viewDidLoad
{
    ...
    self.title = @"펀드조회";
    self.strBackButtonTitle = @"펀드입금예약 1단계";
    ...
    if ([secutryType intValue] == 1 || [secutryType intValue] == 2 ||
        ...
        secretCardViewController.nextSVC = @"D6250";
        ...
        secretOTPViewController.nextSVC = @"D6250";
    ...
    [_securityView setFrame:CGRectMake(0.0f, _infoView.frame.size.height+1, 317.0f, 250.0f)];
...
- (void)getElectronicSignResult:(NSNotification *)notification
{
    ...
    NSLog(@"data_D6020 : %@", data_D6020);
    NSLog(@"self.dicDataDictionary : %@", self.dicDataDictionary);
    [self.dicDataDictionary setObject:[data_D6010 objectForKey:@"계좌명"] forKey:@"펀드명"];
    [self.dicDataDictionary setObject:tempStr1 forKey:@"계좌번호"];
    [self.dicDataDictionary setObject:[data_D6020 objectForKey:@"거래일자"] forKey:@"거래일자"];
    ...
    [self.dicDataDictionary setObject:[data_D6020 objectForKey:@"거래금액"] forKey:@"입금신청금액"];
    [self.dicDataDictionary setObject:[data_D6020 objectForKey:@"신청일자"] forKey:@"입금신청일자"];
    [self.dicDataDictionary setObject:[data_D6010 objectForKey:@"COM_TRAN_DATE"] forKey:@"입금취소일자"];
...
//    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"eSignCancel" object:nil];
    ...
    SHBFundDepositApplyCompleteViewController *detailViewController = [[SHBFundDepositApplyCompleteViewController alloc] initWithNibName:@"SHBFundDepositApplyCompleteViewController" bundle:nil];
    
    detailViewController.dicDataDictionary = self.dicDataDictionary;
...
- (void) confirmSecretMedia:(OFDataSet*)confirmData result:(BOOL)confirm media:(int)mediaType
{
    ...
    if (confirm) {
        ...
        AppInfo.electronicSignCode = @"D6250";
        AppInfo.electronicSignTitle = @"입금예약 취소";
        ...
        [AppInfo addElectronicSign:[NSString stringWithFormat:@"(6)입금신청금액 : %@", [data_D6020 objectForKey:@"거래금액"]]];
//        [AppInfo addElectronicSign:[NSString stringWithFormat:@"[7]입금신청일자 : %@", [data_D6020 objectForKey:@"신청일자"]]];
        [AppInfo addElectronicSign:[NSString stringWithFormat:@"(7)입금취소일자 : %@", [data_D6020 objectForKey:@"처리예정일자"]]];
        ...
        self.service = [[[SHBFundService alloc] initWithServiceId:FUND_DEPOSIT_CANCEL
