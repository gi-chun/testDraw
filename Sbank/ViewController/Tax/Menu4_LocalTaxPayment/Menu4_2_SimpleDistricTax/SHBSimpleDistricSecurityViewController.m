//
//  SHBSimpleDistricSecurityViewController.m
//  ShinhanBank
//
...
#import "SHBSimpleDistricSecurityViewController.h"
#import "SHBGiroTaxListService.h"                   // 지로 지방세 서비스
#import "SHBSimpleDistricTaxCompleteViewController.h"           // 다음 view


@interface SHBSimpleDistricSecurityViewController ()

@end

@implementation SHBSimpleDistricSecurityViewController
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SimpleDistricCancelButtonPushed" object:nil];
...
- (void)getElectronicSignResult:(NSNotification *)noti
{
    if ([noti userInfo])
    {
        ...
        {
            // success의 경우
            SHBSimpleDistricTaxCompleteViewController *viewController = [[SHBSimpleDistricTaxCompleteViewController alloc] initWithNibName:@"SHBSimpleDistricTaxCompleteViewController" bundle:nil];
...
- (void)viewDidLoad
{
    ...
    self.title = @"지방세납부";
    self.strBackButtonTitle = @"간편납부번호 2단계";
    ...
    label4.text = [dicDataDictionary objectForKey:@"납기내납기일"];
    label5.text = [dicDataDictionary objectForKey:@"납기후납기일"];
    label6.text = [NSString stringWithFormat:@"%@원", [dicDataDictionary objectForKey:@"납부금액"]];
