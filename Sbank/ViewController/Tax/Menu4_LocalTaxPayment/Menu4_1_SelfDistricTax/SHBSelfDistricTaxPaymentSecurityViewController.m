//
//  SHBSelfDistricTaxPaymentSecurityViewController.m
//  ShinhanBank
//
//  Created by Jonghyeop Kim on 12. 11. 5..
//  Copyright (c) 2012년 (주)두베. All rights reserved.
//

#import "SHBSelfDistricTaxPaymentSecurityViewController.h"
#import "SHBGiroTaxListService.h"                           // 지로 서비스
#import "SHBDistricTaxCompleteViewController.h"             // 다음 view


@interface SHBSelfDistricTaxPaymentSecurityViewController ()

@end

@implementation SHBSelfDistricTaxPaymentSecurityViewController
...
- (void)setSecretView
{
    ...
    {           //보안카드
        ...
        [secretView setFrame:CGRectMake(secretView.frame.origin.x, secretView.frame.origin.y, secretcardView.view.frame.size.width, secretcardView.view.bounds.size.height)];
        ...
        NSLog(@"secretcardView.view : %@", NSStringFromCGRect(secretcardView.view.frame));
        NSLog(@"secretView.view : %@", NSStringFromCGRect(secretView.frame));
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DistricTaxCancelButtonPushed" object:nil];
...
- (void)getElectronicSignResult:(NSNotification *)noti
{
    if ([noti userInfo])
    {
        ...
        {
            // success의 경우
            SHBDistricTaxCompleteViewController *viewController = [[SHBDistricTaxCompleteViewController alloc] initWithNibName:@"SHBDistricTaxCompleteViewController" bundle:nil];
...
- (void)viewDidLoad
{
    ...
    self.title = @"지방세납부";
    self.strBackButtonTitle = @"지방세납부 2단계";
