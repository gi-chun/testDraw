//
//  SHBSelfDistricTaxPaymentAccountViewController.m
//  ShinhanBank
//
...
#import "SHBSelfDistricTaxPaymentAccountViewController.h"
#import "SHBGiroTaxListService.h"           // 지로 지방세 서비스
#import "SHBAccountService.h"               // 계좌관련 서비스
#import "SHBSelfDistricTaxPaymentSecurityViewController.h"      // 보안카드 view


@interface SHBSelfDistricTaxPaymentAccountViewController ()

@end

@implementation SHBSelfDistricTaxPaymentAccountViewController

#pragma mark -
#pragma mark Synthesize

@synthesize dicDataDictionary;
@synthesize dicAccountDic;
    ...
    switch ([sender tag]) {
        ...
        case 21:            // 확인 버튼
        {
            ...
            SHBDataSet     *dicDataDic = [[[SHBDataSet alloc] initWithDictionary:
                                           ...
                                           @"은행구분"              : [self.dicAccountDic objectForKey:@"은행코드"], // (!주의)신, 구 계좌에 따른 값 변경 있음 정보처리되어서 옴
...
- (void)listPopupView:(SHBListPopupView *)listPopView didSelectedIndex:(NSInteger)anIndex
{
    ...
    {
        self.dicAccountDic = [self.dataList objectAtIndex:anIndex];
        [buttonAccount setTitle:[self.dicAccountDic objectForKey:@"2"] forState:UIControlStateNormal];
...
- (BOOL)onParse:(OFDataSet *)aDataSet string:(NSData *)aContent
{
    if ([[aDataSet objectForKey:@"COM_SVC_CODE"] isEqualToString:@"C2090"])      // 계좌가 확인된 경우
    {
        if ( [[aDataSet objectForKey:@"COM_RESULT_CD"] isEqualToString:@"0"] )
        {
            ...
            SHBSelfDistricTaxPaymentSecurityViewController *viewController = [[SHBSelfDistricTaxPaymentSecurityViewController alloc] initWithNibName:@"SHBSelfDistricTaxPaymentSecurityViewController" bundle:nil];
    ...
    else if ( [[aDataSet objectForKey:@"COM_SVC_CODE"] isEqualToString:@"G1412"] )      // 상세정보
    {
        if ( [[aDataSet objectForKey:@"COM_RESULT_CD"] isEqualToString:@"0"] )
        {
            ...
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"납기내금액"]] forKey:@"납기내금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"납기후금액"]] forKey:@"납기후금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"과세표준금액"]] forKey:@"과세표준금액원"];
            
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목1금액"]] forKey:@"세목1금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목2금액"]] forKey:@"세목2금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목3금액"]] forKey:@"세목3금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목4금액"]] forKey:@"세목4금액원"];
            
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목1가산금액"]] forKey:@"세목1가산금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목2가산금액"]] forKey:@"세목2가산금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목3가산금액"]] forKey:@"세목3가산금액원"];
            [aDataSet setObject:[NSString stringWithFormat:@"%@원", aDataSet[@"세목4가산금액"]] forKey:@"세목4가산금액원"];
                         
            self.dicDataDictionary = aDataSet;              // 정보 전달을 위해
        }
    }
...
- (void)viewDidLoad
{
    ...
    // title 설정
    self.title = @"지방세납부";
    self.strBackButtonTitle = @"지방세납부 1단계";
    ...
    scrollView.contentSize = CGSizeMake(viewRealView.frame.size.width, viewRealView.frame.size.height);
    ...
    OFDataSet *aDataSet = [[[OFDataSet alloc] initWithDictionary:
                            ...
                            @"예비1" : [self.dicDataDictionary objectForKey:@"예비1"],
                            }] autorelease];
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DistricTaxCancelButtonPushed" object:nil];
    // cancel button notification
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cancelButtonDidPush) name:@"DistricTaxCancelButtonPushed" object:nil];
}
