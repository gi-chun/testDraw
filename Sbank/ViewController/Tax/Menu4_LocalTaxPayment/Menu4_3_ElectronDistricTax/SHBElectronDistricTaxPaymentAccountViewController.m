//
//  SHBElectronDistricTaxPaymentAccountViewController.m
//  ShinhanBank
//
...
#import "SHBElectronDistricTaxPaymentAccountViewController.h"
#import "SHBGiroTaxListService.h"               // 지로 서비스
#import "SHBAccountService.h"               // 계좌관련 서비스
#import "SHBElectronDistricTaxPaymentSecurityViewController.h"          // 다음 view


@interface SHBElectronDistricTaxPaymentAccountViewController ()

@end

@implementation SHBElectronDistricTaxPaymentAccountViewController
...
- (IBAction)buttonDidPush:(id)sender
{
    switch ([sender tag]) {
        ...
        case 22:            // 취소
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ElectronDistricCancelButtonPushed" object:nil];
...
#pragma mark -
#pragma mark onParse & onGenerate
...
- (BOOL)onParse:(OFDataSet *)aDataSet string:(NSData *)aContent
{
    if ([[aDataSet objectForKey:@"COM_SVC_CODE"] isEqualToString:@"C2090"])      // 계좌가 확인된 경우
    {
        if ( [[aDataSet objectForKey:@"COM_RESULT_CD"] isEqualToString:@"0"] )
        {
            ...
            SHBElectronDistricTaxPaymentSecurityViewController *viewController = [[SHBElectronDistricTaxPaymentSecurityViewController alloc] initWithNibName:@"SHBElectronDistricTaxPaymentSecurityViewController" bundle:nil];
    ...
    else if ( [[aDataSet objectForKey:@"COM_SVC_CODE"] isEqualToString:@"G1412"] )           // 납부금액 판단
    {
        if ( [[aDataSet objectForKey:@"COM_RESULT_CD"] isEqualToString:@"0"] )
        {
            if ( [[aDataSet objectForKey:@"납기내후구분"] isEqualToString:@"B"] )
            {
            ...
            [self.dicDataDictionary removeAllObjects];          // data를 넘기기 위해
...
- (void)viewDidLoad
{
    ...
    self.title = @"지방세납부";
    self.strBackButtonTitle = @"전자납부번호 1단계";
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"ElecDistricCancelButtonPushed" object:nil];
    // cancel button notification
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cancelButtonDidPush) name:@"ElecDistricCancelButtonPushed" object:nil];
}
