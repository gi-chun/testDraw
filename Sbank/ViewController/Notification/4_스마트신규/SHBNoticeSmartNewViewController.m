//
//  SHBNoticeSmartNewViewController.m
//  ShinhanBank
//
...
#import "SHBNoticeSmartNewViewController.h"
#import "SHBNoticeSmartNewCell.h" // cell
...
@interface SHBNoticeSmartNewViewController ()

@property (retain, nonatomic) NSMutableDictionary *selectDic;

@end

@implementation SHBNoticeSmartNewViewController
...
- (void)setD3249Data:(NSMutableArray *)array
{
    ...
    for (SHBDataSet *dataSet in array) {
        ...
        [dataSet insertObject:@"N" forKey:@"isOpen" atIndex:0];
...
- (void)newProductJoin
{
    // SHBSmartNewListViewController, SHBSmartNewListViewController 동일하게 수정 필요
...
- (void)tableViewJoinPressed:(UIButton *)sender
{
    ...
    // 하단 내용 수정시 SHBSmartNewListViewController 동일하게 수정 필요
...
- (void)tableViewArrowPressed:(UIButton *)sender
{
    OFDataSet *cellDataSet = self.dataList[sender.tag];
    
    if ([cellDataSet[@"isOpen"] isEqualToString:@"Y"]) {
        
        [cellDataSet insertObject:@"N" forKey:@"isOpen" atIndex:0];
    }
    else {
        
        [cellDataSet insertObject:@"Y" forKey:@"isOpen" atIndex:0];
    }
...
- (BOOL)onParse:(OFDataSet *)aDataSet string:(NSData *)aContent
{
    ...
    if (self.service.serviceId == SMART_NEW_LIST) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cellDataSet[@"isOpen"] isEqualToString:@"N"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cellDataSet[@"isOpen"] isEqualToString:@"N"]) {
        ...
        SHBNoticeSmartNewCell *cell = (SHBNoticeSmartNewCell *)[tableView dequeueReusableCellWithIdentifier:@"SHBNoticeSmartNewCell"];
        
        if (!cell) {
            
            NSArray *array = [[NSBundle mainBundle] loadNibNamed:@"SHBNoticeSmartNewCell"
                                                           owner:self options:nil];
            
            cell = (SHBNoticeSmartNewCell *)array[0];
        ...
        [cell.arrowBtn setHidden:NO];
        [cell.arrowBtn setTag:indexPath.row];
        [cell.arrowBtn addTarget:self action:@selector(tableViewArrowPressed:) forControlEvents:UIControlEventTouchUpInside];
        
        if ([cellDataSet[@"serviceCode"] isEqualToString:@"D3249"]) {
            
            // 금리우대설계서
            
            [cell.noticeTitle setText:@"금리우대설계서"];
            
            [cell.noticeMessage initFrame:cell.noticeMessage.frame colorType:RGB(74, 74, 74) fontSize:15 textAlign:0];
            [cell.noticeMessage setCaptionText:[NSString stringWithFormat:@"(유효일자:%@ / %@)",
                                                cellDataSet[@"만기일"], cellDataSet[@"_상담직원"]]];
        }
        else if ([cellDataSet[@"serviceCode"] isEqualToString:@"D3251"]) {
            
            if ([cellDataSet[@"스마트신규구분"] isEqualToString:@"1"]) {
                
                [cell.noticeTitle setText:@"예적금맞춤설계서"];
            }
            else if ([cellDataSet[@"스마트신규구분"] isEqualToString:@"2"]) {
                
                [cell.noticeTitle setText:@"만기후 맞춤설계서"];
            }
            else {
            ...
            [cell.noticeMessage initFrame:cell.noticeMessage.frame colorType:RGB(74, 74, 74) fontSize:15 textAlign:0];
            [cell.noticeMessage setCaptionText:[NSString stringWithFormat:@"(유효일자:%@ / %@ %@)",
                                                cellDataSet[@"_유효기간"], cellDataSet[@"등록지점명"], cellDataSet[@"등록직원명"]]];
        }
        else if ([cellDataSet[@"serviceCode"] isEqualToString:@"D6170"]) {
            
            // 펀드맞춤설계서
            
            [cell.noticeTitle setText:cellDataSet[@"_설계서명"]];
            
            [cell.noticeMessage initFrame:cell.noticeMessage.frame colorType:RGB(74, 74, 74) fontSize:15 textAlign:0];
            [cell.noticeMessage setCaptionText:[NSString stringWithFormat:@"(유효일자:%@ / %@ %@)",
                                                cellDataSet[@"_설계서유효기간"], cellDataSet[@"등록지점명"], cellDataSet[@"등록직원명"]]];
        }
        ...
        if ([cellDataSet[@"serviceCode"] isEqualToString:@"D3249"]) {
            ...
            if (cellDataSet[@"_상품명높이"]) {
                ...
                FrameResize(cell.arrowBtn, width(cell.arrowBtn), height(cell.arrowBtn) + productHeight - 16);
            }
            ...
            if ([cellDataSet[@"스마트신규구분"] isEqualToString:@"1"]) {
                ...
                if ([cellDataSet[@"상품코드"] hasPrefix:@"209"]) {
                    ...
                    if ([cellDataSet[@"계약기간"] isEqualToString:@"0"]) {
                        ...
                        FrameResize(cell.arrowBtn, width(cell.arrowBtn), height(cell.arrowBtn) - 25);
                    }
                ...
                */
                ...
                FrameResize(cell.arrowBtn, width(cell.arrowBtn), height(cell.arrowBtn) + arrowHeight);
