//
//  SHBLoanInterestPayComfirmViewController.m
//  ShinhanBank
//
...
@interface SHBLoanInterestPayComfirmViewController ()<SHBSecretCardDelegate, SHBSecretOTPDelegate>
{
...
@implementation SHBLoanInterestPayComfirmViewController
...
- (void) confirmSecretMedia:(OFDataSet*)confirmData result:(BOOL)confirm media:(int)mediaType
{
    ...
    AppInfo.electronicSignCode = @"L1231";
    AppInfo.electronicSignTitle = AppInfo.commonDic[@"제목"];
    
    self.service = [[[SHBLoanService alloc] initWithServiceCode:@"L1231" viewController:self] autorelease];
    
    SHBDataSet *aDataSet = [[[SHBDataSet alloc] initWithDictionary:@{
                             @"한도대출계좌번호" : [AppInfo.commonDic[@"한도대출계좌번호"] stringByReplacingOccurrencesOfString:@"-" withString:@""],
                             @"은행구분" : @"1",
                             @"출금계좌비밀번호" : AppInfo.commonDic[@"출금계좌비밀번호"],
                             @"이자금액" : [SHBUtility commaStringToNormalString:AppInfo.commonDic[@"이자금액"]],
                             }] autorelease];
...
{
    SHBLoanInterestPayCompleteViewController *nextViewController = [[[SHBLoanInterestPayCompleteViewController alloc] initWithNibName:@"SHBLoanInterestPayCompleteViewController" bundle:nil] autorelease];
...
- (void)viewDidLoad
{
    ...
    { //otp 사용
        ...
        secretotpView.nextSVC = @"L1231";
        
        _secretView.frame = CGRectMake(0.0f, 136.0f, 317.0f, secretotpView.view.frame.size.height);
    ...
    else
    {
        ...
        secretcardView.nextSVC = @"L1231";
        
        _secretView.frame = CGRectMake(0.0f, 136.0f, 317.0f, secretcardView.view.frame.size.height);
        _infoView.frame = CGRectMake(0, 0, 317, 136 + _secretView.frame.size.height);
