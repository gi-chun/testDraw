//
//  SHBCelebrationTransferComfirmViewController.m
//  ShinhanBank
//
...
@interface SHBCelebrationTransferComfirmViewController ()<SHBSecretCardDelegate, SHBSecretOTPDelegate>
{
...
@implementation SHBCelebrationTransferComfirmViewController
...
- (void) confirmSecretMedia:(OFDataSet*)confirmData result:(BOOL)confirm media:(int)mediaType
{
    ...
    if([AppInfo.commonDic[@"전문번호"] isEqualToString:@"D2021"])
    {
        AppInfo.electronicSignCode = @"D2023";
        AppInfo.electronicSignTitle = AppInfo.commonDic[@"제목"];
        
        self.service = [[[SHBAccountService alloc] initWithServiceCode:@"D2023" viewController:self] autorelease];
...
{
    SHBCelebrationTransferCompleteViewController *nextViewController = [[[SHBCelebrationTransferCompleteViewController alloc] initWithNibName:@"SHBCelebrationTransferCompleteViewController" bundle:nil] autorelease];
...
- (void)viewDidLoad
{
    ...
    self.title = @"기타이체";
    [self.view addSubview:[[[SHBGoodsSubTitleView alloc]initWithTitle:@"경조금 이체정보 확인" maxStep:3 focusStepNumber:2] autorelease]];
    ...
    _lblData08.text = AppInfo.commonDic[@"받는통장메모"];
    _lblData09.text = AppInfo.commonDic[@"내통장메모"];
